...
describe('/api/onboarding/message API Route', () => {
  ...
  test('should successfully complete the onboarding flow (Happy Path - Explorer Path)', async () => {
    ...
    mockCreateSession.mockResolvedValue({ sessionId: MOCK_SESSION_ID, initialState: { ...currentSessionState } }); // Initial state for new session
    ...
    mockUpdateSession.mockImplementation(async (sessionId: string, newState: SessionState) => {
      if (sessionId === MOCK_SESSION_ID) {
    ...
    request = createMockRequest({ sessionId: MOCK_SESSION_ID, response: 'Test User' });
    ...
    // --- Simulation Step 3: Answer Q1 (Email) ---
    console.log('[Test Step 3] Answering Q1...');
    request = createMockRequest({ sessionId: MOCK_SESSION_ID, response: 'test@example.com' });
    response = await POST(request);
    body = await response.json();
    ...
    expect(body.finalResult?.recommendedPath).toBe(MOCK_FINAL_PATH.recommendedPath);
    expect(body.finalResult?.recommendedPathUrl).toBe(MOCK_FINAL_PATH.recommendedPathUrl);
    expect(body.error).toBeNull();
  ...
  test('should handle session expiry by forcing a restart', async () => {
    ...
    const FIRST_QUESTION = { index: 0, text: 'Q0: Restart Name?', inputMode: 'text' };

    // Mock getSession to return null for the expired ID
    mockGetSession.mockResolvedValue(null);
    // Mock createSession to be called when restarting
    mockCreateSession.mockResolvedValue({ sessionId: NEW_SESSION_ID, initialState: { questionIndex: 0, accumulatedData: {}, repromptedIndex: null, lastInteractionTimestamp: Date.now() } });
    // Mock getQuestionDetails for the first question
    mockGetQuestionDetails.mockImplementation((index: number) => index === 0 ? FIRST_QUESTION : null);
    // Mock isFinalQuestion for index 0
    mockIsFinalQuestion.mockReturnValue(false);

    const request = createMockRequest({ sessionId: EXPIRED_SESSION_ID, response: 'anything' });
    const response = await POST(request);
    const body = await response.json();
  ...
  test('should re-prompt on first validation failure if message exists', async () => {
    ...
    const QUESTION_DETAIL = { index: QUESTION_INDEX, text: 'Q0: Name?', inputMode: 'text', validationHint: 'text', rePromptMessage: 'Please enter a valid name.' };
    const initialState: SessionState = { questionIndex: QUESTION_INDEX, accumulatedData: {}, repromptedIndex: null, lastInteractionTimestamp: Date.now() };
    ...
    expect(mockUpdateSession).toHaveBeenCalledWith(SESSION_ID, expect.objectContaining({ repromptedIndex: QUESTION_INDEX }));
    // Check response body
  ...
  test('should halt flow on second email validation failure', async () => {
    ...
    const QUESTION_INDEX = 1; // Email question
    const QUESTION_DETAIL = { index: QUESTION_INDEX, text: 'Q1: Email?', inputMode: 'text', validationHint: 'email', rePromptMessage: 'Invalid email format.' };
    // Simulate state *after* first failure (repromptedIndex is set)
    const initialState: SessionState = { questionIndex: QUESTION_INDEX, accumulatedData: { name: 'Test' }, repromptedIndex: QUESTION_INDEX, lastInteractionTimestamp: Date.now() };
    ...
    expect(body.error).toBe('A valid email address is required. Please refresh to start over.');
  ...
  describe('GET Handler', () => {
    test('should return OK status when services connect', async () => {
      ...
      mockCreateSession.mockResolvedValue({ sessionId: MOCK_SESSION_ID, initialState: {} });
      mockGetSession.mockResolvedValue({ questionIndex: 0 /* minimal state */ });
    ...
    test('should return ERROR status if getSession fails post-create', async () => {
      ...
      expect(body.message).toContain('Session Service failed post-create check');
    });
