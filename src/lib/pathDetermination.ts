...
function getPathUrl(pathKey: string): string {
  ...
  console.warn(
    `Environment variable ${envVarName} not set. Using application routes.`,
  );
  ...
  const route =
    pathRoutes[pathKey] || `/${pathKey.toLowerCase().replace(/ /g, "-")}`;
...
function hasContractorRequirements(data: OnboardingData): boolean {
  ...
  return (
    ...
    !!blockchains &&
    blockchains.includes("Cosmos SDK Chains")
  );
}
...
export function determinePath(data: OnboardingData): PathResult {
  ...
  const secondaryUrl = secondBest?.path
    ? getPathUrl(secondBest.path)
    : undefined;
  ...
  const secondRecommendedPathDescription = secondBest?.path
    ? PATH_DESCRIPTIONS[secondBest.path] || undefined
    : undefined;
