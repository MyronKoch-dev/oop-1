...
if (!supabaseServiceRoleKey) {
    throw new Error('Supabase environment variable SUPABASE_SERVICE_ROLE_KEY is not configured!');
}
 ...
 */
export async function saveOnboardingResponse(data: OnboardingData): Promise<{ success: boolean; error?: string }> {
    ...
    if (!data.email || typeof data.email !== 'string' || !data.email.includes('@')) {
        const errorMsg = 'Critical: Attempted to save onboarding response without a valid email.';
        console.error(errorMsg, { emailProvided: data.email });
        return { success: false, error: 'Email is required and must be valid to save the response.' };
    }
    ...
    console.log(`Attempting to save data to Supabase for email: ${dbPayload.email}`);
    ...
    while (attempts < maxAttempts) {
        ...
        const { error } = await supabaseAdmin
            .from(TABLE_NAME)
            .insert(dbPayload); // Insert the correctly typed payload

        // --- Handle Success ---
        if (!error) {
            console.log(`Successfully saved onboarding response for email: ${dbPayload.email} on attempt ${attempts}`);
            return { success: true };
        }

        // --- Handle Errors ---
        console.warn(`Supabase save attempt ${attempts} failed:`, error.message, `Code: ${error.code}`, `Hint: ${error.hint}`);

        // Specific Error Handling: Unique Constraint (Email already exists)
        if (error.code === '23505') { // Standard PostgreSQL unique violation code
            console.error(`DB Unique constraint violation (email likely already exists): ${dbPayload.email}`);
            // Return success=false but with a specific error message
            return { success: false, error: `Email already exists: ${dbPayload.email}. ${error.hint}` };
        }

        // Specific Error Handling: Table doesn't exist (common setup issue)
        if (error.code === '42P01') { // Standard PostgreSQL undefined table code
            console.error(`DB Error: Table '${TABLE_NAME}' does not exist. Check Supabase schema setup.`);
            return { success: false, error: `Database table not found: ${error.message}` };
        }

        // --- Handle Retry ---
        if (attempts >= maxAttempts) {
            console.error(`Supabase save failed after ${maxAttempts} attempts for email ${dbPayload.email}:`, error);
            return { success: false, error: `DB save failed after multiple attempts: ${error.message}` };
        }

        // Calculate delay with exponential backoff and jitter
        const delay = baseDelay * Math.pow(2, attempts - 1) * (1 + Math.random() * 0.2 - 0.1);
    ...
    const finalErrorMsg = 'Unknown error occurred during the database save process after retries.';
