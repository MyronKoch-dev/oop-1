...
if (!redisUrl) {
  throw new Error(
    "Session Service Error: UPSTASH_REDIS_REST_URL environment variable is not configured!",
  );
}
if (!redisToken) {
  throw new Error(
    "Session Service Error: UPSTASH_REDIS_REST_TOKEN environment variable is not configured!",
  );
}
...
export async function createSession(): Promise<{
  ...
  try {
    ...
    if (result !== "OK") {
      throw new Error(
        `Redis SET command failed for new session. Result: ${result}`,
      );
    }

    console.log(
      `[Session: ${sessionId}] Session created successfully with TTL ${SESSION_TTL_SECONDS}s`,
    );
    return { sessionId, initialState }; // Return the new ID and the initial state object
  } catch (error) {
    console.error(
      `[Session: ${sessionId}] Redis error during session creation:`,
      error,
    );
...
export async function getSession(
  ...
  try {
    ...
    if (data) {
      ...
      console.log(
        `[Session: ${sessionId}] Session data not found (likely expired or invalid ID).`,
      );
      return null;
    }
  } catch (error) {
    console.error(
      `[Session: ${sessionId}] Redis error during getSession:`,
      error,
    );
...
export async function updateSession(
  ...
  try {
    ...
    if (result !== "OK") {
      throw new Error(
        `Redis SET command failed during session update. Result: ${result}`,
      );
    }
    console.log(
      `[Session: ${sessionId}] Session updated successfully, TTL reset.`,
    );
  } catch (error) {
    console.error(
      `[Session: ${sessionId}] Redis error during updateSession:`,
      error,
    );
    throw new Error(
      `Failed to update session in Redis: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function deleteSession(sessionId: string): Promise<void> {
  ...
  try {
    ...
    if (deletedCount > 0) {
      console.log(
        `[Session: ${sessionId}] Session deleted successfully from Redis.`,
      );
    } else {
      // This is not necessarily an error, the key might have already expired or been deleted
      console.log(
        `[Session: ${sessionId}] Session delete command executed, but key was not found (already expired/deleted?).`,
      );
    }
  } catch (error) {
    // Log Redis errors during deletion but don't interrupt application flow
    console.error(
      `[Session: ${sessionId}] Redis error during deleteSession:`,
      error,
    );
  }
}
